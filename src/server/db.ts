...
import { IDataWrapper, sortDsc, sortAsc } from './dataWrapper';
...
class Database {
  ...
  retrieveLikeForIp(_ip) {
    const like = this.dataWrapper.findLike({ _ip });
    if (!like) return null;
    return like.timestamp;
  }

  isCommentWithValidFrequency(article, commentData: CommentAttributes) {
  ...
  submitComment(id: string, commentData: CommentAttributes) {
    try {
      const article = this.getArticle(id);
      if (!article) {
        return [ false, null ];
      ...
      const isValidPostFrequency = this.isCommentWithValidFrequency(article, commentData);
...
Article.isCommentWithValidFrequency = async function(article, commentData: CommentAttributes) {
  const [ com1, com2 ] = await article.getComments({
    where: { _ip: commentData._ip },
...
export { Article, Like, Comment, init, sequalize, Database };
