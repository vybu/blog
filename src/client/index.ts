...
// TODO: add event listeners for the newly rendered content
...
function startFetchingJson({ href }: RoutingInfo): EventListenerOrEventListenerObject {
    return () => {
        contentPromises[href] = {
            then(handler) {
                getJSON(href).then(handler).catch(err => console.error(err));
            }
        };
    };
}
...
function changeUrlTo({ href, containerId }: RoutingInfo, innerHTML: innerHTML, replace: boolean = false) {
    console.info(href, replace, innerHTML);
...
function handleHistoryChange({ containerId, innerHTML }: HistoryState) {
    const container = getContainerElement(containerId);

    if (container) {
        console.log('updating container');
        container.innerHTML = innerHTML;
        initRouter(`#${containerPrefix}${containerId} ${ROUTER_LINKS_SELECTOR}`, articleJsonLoadHandler);
    } else {
...
function articleJsonLoadHandler(r: ParsedArticle) {
    return r.articleHtml;
}
...
initRouter(ROUTER_LINKS_SELECTOR, articleJsonLoadHandler);
