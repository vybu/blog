...
export function newComment(articleId: string, parentId: string = null) {
    return `
        <form class="new-comment" action="${isDevMode ? `http://localhost:${serverPort}` : ''}/comments/${articleId}" method="post">
            <input style="display: none" name="parent" type="text" value="${parentId || articleId}"></input>
            <textarea class="comment" name="comment" placeholder="Your comment goes here"></textarea>
            <div class="bottom">
